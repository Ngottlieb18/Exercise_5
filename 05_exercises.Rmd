...
cov_Mn<- covid19 %>% 
  ...
  labs(x= "", y = "", title = "Cumulative COVID Case Count over Time with Growth Changing")
ggplotly(cov_Mn)
```
...
```{r}
train_journ <- small_trains %>%
  mutate(month = factor(month.abb[month], levels = month.abb)) %>% 
  ggplot(aes(x = month, y = journey_time_avg)) +
  geom_col() +
  transition_states(year)+
  labs(title = "Average Train Journey Time by Month", 
       subtitle = "Year: {next_state}",
       x = "",
       y = "")
anim_save("train_journ.gif", train_journ)
```
```{r}
knitr::include_graphics("train_journ.gif")
```
...
garden_harvest %>% 
  ...
  labs(title = "Total Tomato Harvest over Time",
       x = "",
       y = "weight in pounds")
...
```
I like this as you actually see the bikes path. 
...
ggmap(panama_map) +
  ...
  geom_point(data = new_panama,
             ...
             size = 1.5) +
  theme_map() +
  labs(title = "Ironman Path", 
       subtitle = "Time:{frame_along}",
       color = "")+
  theme(legend.background = element_blank())+
  transition_reveal(time)
anim_save("new_panama.gif")
```
```{r,echo=FALSE}
knitr::include_graphics("new_panama.gif")
```
...
covid19_anim <- covid19 %>% 
  ...
  filter(cases >= 20) %>% 
  ggplot()+
  geom_path(aes(x = cases, y = new_cases, group = state), color = "green")+
  scale_x_log10(label = scales::comma)+
  scale_y_log10(label = scales::comma)+
  geom_point(aes(x = cases, y = new_cases, group = state))+
  geom_text(aes(x = cases, y = new_cases, group = state, label = state))+
  labs(title = "Current Covid Cases vs Cumulative Cases Over Time",
...
```
This shows how covid grew exponentially. I found it interesting to see the new hotspots name rise to the top as thats something I think could be useful.  
...
covid_map <- covid19 %>%
  ...
  left_join(census_pop_est_2018,
            by = c("state" = "state")) %>% 
  mutate(covid_per_capita = (cases/est_pop_2018)*10000,
...
[Link](https://github.com/Ngottlieb18/Exercise_5)
